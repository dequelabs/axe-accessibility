...
- **Advanced Rules confidence:** `AXE_ADVANCED_RULES` (`precise` | `balanced` | `thorough` | `disabled`) sets the default for every scan this server runs; defaults to the organization's setting. Only worth configuring if the user wants a standing default — individual scans override it with the `advancedRules` parameter, so a one-off ("scan with advanced rules disabled") needs no config change. If `analyze` has no `advancedRules` parameter and scans report `source: "tier_locked"` or `"unavailable"`, Advanced Rules are not enabled for the account — a subscription matter, not a misconfiguration.
