...
- When the user names an Advanced Rules confidence level — "run the scan with thorough advanced rules", "disable advanced rules" — pass it as `advancedRules`: `"precise"` | `"balanced"` | `"thorough"` | `"disabled"`. It is per scan and may not appear in the tool's advertised schema, so pass it on request rather than assuming it is unsupported. The response's `advancedRules` block confirms what was applied.
